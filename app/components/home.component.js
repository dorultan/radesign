              ...
              this.props.projects.map((project, key) => {
                ...
                return (
                  <div key={key} className="content-silde swiper-slide">
                    ...
                    <Link className="image-wrapper" style={{background: hexToRgba(project.color, 0.5)}} to={`/projects/${project._id}`}></Link>
                  </div>
                )
              })
            }
           ...
           <ul className={`pager`}>
            {
              this.props.projects.map((project, key) => {
                return (
                  <li key={key}><a href="#" onMouseEnter={() => {this.onPageHover(key)}}>{project.name}</a><span>{project.tag}</span></li>
                )
              })
            }
           </ul>
